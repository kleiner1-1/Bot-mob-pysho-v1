...
defaultQueryTimeoutMs: undefined,version: [2, 3000, 1023223821],
}
...
console.log('🌠 Mobpysho no tiene Sub-Bots vinculados.');
return;
}
...
console.log(chalk.bold.greenBright(`🌠 Ai MobPysho conectado correctamente.`))

await connectSubBots();
} catch (error) {
console.error(chalk.bold.cyanBright(`🥀 Error al iniciar MobPysho-Ai: `, error))
}
})();
...
setInterval(() => {
  if (process.send) {
    console.log('⏰ Reinicio automático ejecutado cada 45 minutos');
    process.send('reset');
  }
}, 1000 * 60 * 45);
